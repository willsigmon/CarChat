...
struct GradientIcon: View {
    ...
    init(
        systemName: String,
        gradient: LinearGradient = CarChatTheme.Gradients.accent,
        size: CGFloat = 56,
        iconSize: CGFloat = 24,
        glowColor: Color = CarChatTheme.Colors.glowCyan,
    ...
    var body: some View {
        ...
        .onAppear {
            if isAnimated {
                withAnimation(CarChatTheme.Animation.breathe) {
                    isBreathing = true
                }
            }
        }
...
struct ProviderIcon: View {
    ...
    var body: some View {
        ZStack {
            Circle()
                .fill(CarChatTheme.Colors.providerColor(provider).opacity(0.15))
                .frame(width: size, height: size)
                .overlay(
                    Circle()
                        .strokeBorder(
