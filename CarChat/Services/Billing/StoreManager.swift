...
final class StoreManager {
    ...
    static let subscriptionProductIDs: Set<String> = [
        ...
        "carchat.premium.monthly",
    ]

    static let creditProductIDs: Set<String> = [
        "carchat.credits.50standard",
        "carchat.credits.100standard",
        "carchat.credits.50premium",
        "carchat.credits.100premium",
    ]
    ...
    func updateCurrentEntitlements() async {
        ...
        if activeSubs.contains(where: { $0.id == "carchat.premium.monthly" }) {
            currentTier = .premium
        } else if activeSubs.contains(where: { $0.id == "carchat.standard.monthly" }) {
    ...
    private func tierForProduct(_ productID: String) -> SubscriptionTier {
        switch productID {
        case "carchat.premium.monthly": .premium
        case "carchat.standard.monthly": .standard
        default: .free
        }
    }
}
